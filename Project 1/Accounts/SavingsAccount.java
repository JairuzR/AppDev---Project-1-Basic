...
public class SavingsAccount extends Account implements FundTransfer,Deposit, Withdrawal {
    ...
    public boolean transfer(Bank bank, Account account, double amount) throws IllegalAccountType {
        ...
        if (amount > getBank().getWithdrawLimit()) {
            ...
            String description = String.format("Transfer $%f from Account No.%s to %s Account No.%s", amount, this.getAccountNumber(), bank.getName(), recipient.getAccountNumber());
            addNewTransaction(this.getAccountNumber(), Transaction.Transactions.FundTransfer, description);

            //Saving transaction in the recipient's logs
            String recipientDescription = "Transfer from " + this.getAccountNumber();
            recipient.addNewTransaction(this.getAccountNumber(), Transaction.Transactions.Deposit, recipientDescription);
    ...
    public boolean transfer(Account account, double amount) throws IllegalAccountType {
        ...
        if (recipient == null) {
            ...
            String description = String.format("Transfer $%f from Account No.%s to Account No.%s", amount, this.getAccountNumber(), recipient.getAccountNumber());
            addNewTransaction(this.getAccountNumber(), Transaction.Transactions.FundTransfer, description);

            // Saving the transaction in the recipient's transaction log
//            String recipientDescription = "Transfer from " + this.getAccountNumber();
